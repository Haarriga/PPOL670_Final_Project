...
# varying the regularization penalty
...
# create a workflow using our updated linear regression model
# and the same recipe defined above
lasso_wf <- workflow() %>%
  add_recipe(data_rec) %>%
  add_model(lasso_mod) 

# perform hyperparameter tuning using the lasso_grid
# and the cross_validation folds created above
lasso_cv <- lasso_wf %>%
  tune_grid(
    resamples = folds,
    grid = lasso_grid)
...
# use the finalize_workflow() function with our lasso workflow
# and the best model to update our workflow
